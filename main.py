...
#Средние значения отклика функции по рядкам
...
print("Среднее значения отклика функции y¯1 = "+ str(round(y1av1,2)))
print("Значение, полученые при подсчёте через значения факторов из матрицы планирования: " + str(round(b0 + b1*X1[0] + b2*X2[0] + b3*X3[0],2)))
print("Среднее значения отклика функции y¯2 = "+ str(round(y2av2,2)))
print("Значение, полученые при подсчёте через значения факторов из матрицы планирования: " + str(round(b0 + b1*X1[1] + b2*X2[1] + b3*X3[1],2)))
print("Среднее значения отклика функции y¯3 = "+ str(round(y3av3,2)))
print("Значение, полученые при подсчёте через значения факторов из матрицы планирования: " + str(round(b0 + b1*X1[2] + b2*X2[2] + b3*X3[2],2)))
print("Среднее значения отклика функции y¯4 = "+ str(round(y4av4,2)))
print("Значение, полученые при подсчёте через значения факторов из матрицы планирования: " + str(round(b0 + b1*X1[3] + b2*X2[3] + b3*X3[3],2)))
print("Проверка пройдена\n")

print("Дисперсия по рядкам")
...
f3 = f1*f2
ttabl  = 2.306
print("f3 = f1*f2 = 2.306 (c таблицы)")
if (t0<ttabl):
    print("b0 не значимый")
    b0=0
if (t1<ttabl):
    print("b1 не значимый")
    b1=0
if (t2<ttabl):
    print("b2 не значимый")
    b2=0
if (t3<ttabl):
    print("b3 не значимый")
    b3=0


'''Незначимость критериев (незначимые критерии принимаем за 0) влияет на значения yy*,
которые влияют на переменную sad, которая влияет на переменную Fp (F-критерий Фишера),
которая в свою очередь определяет адекватность уравнения оригиналу'''
...
print("d1=", round(d1,2), "\nd2=", round(d2,2), "\nd3=", round(d3,2), "\nd4=", round(d4,2), "\nd5=", round(sb,2))
print("Fp=", round(Fp,2))
print('Ft берём с таблицы (8 ряд.; 2 столб.)\nFt = 4.5')
Ft=4.5
if Fp>Ft:
